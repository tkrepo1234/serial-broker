...
- Writes from *different* participants have no defined relative order. Establishing one would
...
A write that has been accepted but *not yet started* by the owner is different:
...
a result or when a *new* owner announces itself - which is the signal that the previous one is
