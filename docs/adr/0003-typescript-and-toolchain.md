...
- **tsup** (esbuild) for bundling, with two entry points: the library and the worker script.
  It produces ESM + CJS + `.d.ts` with no configuration archaeology.
...
- **Playwright / real Chromium for all tests**: gives genuine `SharedWorker` and Web Locks
  ...
  simulate a device being unplugged mid-write. Kept as a manual and future end-to-end layer,
  rejected as the primary layer.
