...
| `session.term-flood`                      | warn  | More times of holding the port were named at once than are checked; the rest. Once.        |
| `session.term-check-failed`               | warn  | The browser refused the lock request that checks a term; the message was ignored.          |
