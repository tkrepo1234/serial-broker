...
| `supervisor.write-stalled`                | warn  | The device did not take a write within `writeTimeoutMs`. The write stays in flight and holds the queue, and the connection stays open (ADR-0013); the tab holding the port reports `stalledWriteSince` until it settles. |
| `supervisor.write-expired`                | debug | A write waited `writeTimeoutMs` behind others and was not begun; `queuedWrites` remain.                                                                                                                                  |
| `supervisor.write-not-approved`           | debug | The tab that issued a write answered that it had given it up, or did not answer within `writeTimeoutMs` of the write reaching the port; the write was not begun (ADR-0013).                                              |
