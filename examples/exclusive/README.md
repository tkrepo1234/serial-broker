...
re-running the connection would interrupt a working port - and tries a failed one again, in
whichever tab holds the port. `useTheDevice()` in [`src/main.ts`](src/main.ts) relies on that. Two
