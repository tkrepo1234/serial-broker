...
**The same application in JavaScript** is [`examples/openui5-js`](../openui5-js/README.md):
classic `sap.ui.define` modules and `.extend()`, with no transpile step - the shape of a UI5
project that was never migrated to TypeScript. The two show the same screens and the same module.
...
will report that there is nothing to do once the package ships the fix.
