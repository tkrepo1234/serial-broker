/**
 * The SAPUI5 JavaScript example, driven the way a user drives it: the page loads, _Connect_ is
 * clicked, the port opens, a line is sent and the loopback device echoes it into the traffic list.
 *
 ...
 * ```
 *
 * The same test as `examples/openui5/smoke.spec.ts`, against the same screens: what differs is the
 * application under it - classic `sap.ui.define` modules, served without a transpile step - and
 * that is precisely what a smoke test of this example has to prove still works.
 *
...
test.describe('the SAPUI5 JavaScript example', () => {
