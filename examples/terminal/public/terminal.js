 ...
 *
 * The only "build" is the import map in index.html. The JSDoc types are checked by
 * `npm run typecheck` against the library's published declarations, so a misspelt option or event
 * name is caught without a compile step.
 *
...
import { isSerialBrokerError, isSupported, SerialBroker, SerialBrokerStatus } from 'serial-broker';

/** The configuration every tab of this terminal shares. The name is what they have in common. */
const NAME = 'Terminal';

/** Where serve.mjs puts the broker script. Every tab must name the same URL (ADR-0006). */
...
function showError(failure) {
  ...
  if (isSerialBrokerError(failure)) {
    ...
    el('error-message').textContent = failure instanceof Error ? failure.message : String(failure);
...
async function start() {
  ...
  if (!isSupported()) {
    renderStatus('failed');
    append(
      'Web Serial is not available here. Use Chrome or Edge, over https or localhost.',
      'note',
    );
    return;
  }
