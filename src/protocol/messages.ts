...
export interface HelloMessage extends Envelope {
  readonly type: 'hello';
  /**
   * A secret the sender proves its identity to the worker with (ADR-0028).
   *
   * Random, generated by the transport, and sent in this message and nowhere else. The worker binds
   * an identity to the first secret it is shown and refuses a later `hello` naming that identity
   * with another one, so no other script of the origin can speak on the worker as a tab whose
   * identity it heard on the bus - while the tab itself, connecting again to a worker that hung,
   * shows the same secret and is served.
   *
   * `undefined` on `BroadcastChannel`, where every context of the origin receives every message and
   * a secret would be no secret. The worker refuses a `hello` that carries none.
   */
